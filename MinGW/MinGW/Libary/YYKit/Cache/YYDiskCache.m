...
// DL_ADD_YYWebImage
- (void)stt_setObject:(id<NSCoding>)object forKey:(NSString *)key {
    if (!key) return;
    if (!object) {
...
- (NSString *)stt_fileNameWithUrl:(NSString *)urlstr {
    if (!urlstr) {
        return nil;
    }
    return [self _filenameForKey:urlstr];
}
- (NSString *)stt_absolutePathForKey:(NSString *)key {
    if (!key) {
        return nil;
    }
    return [_kv.stt_filePath stringByAppendingPathComponent:[self _filenameForKey:key]];
}
